...
func (h *Handler) databaseStartUp() {
	...
	_, err = h.DB.Exec(`CREATE TABLE IF NOT EXISTS products (id INTEGER PRIMARY KEY AUTOINCREMENT, producttypeid INTEGER REFERENCES product_type(id),  productattributeid INTEGER REFERENCES product_attribute(id), attributevalue TEXT)`)
	if err != nil {
		fmt.Println(err)
